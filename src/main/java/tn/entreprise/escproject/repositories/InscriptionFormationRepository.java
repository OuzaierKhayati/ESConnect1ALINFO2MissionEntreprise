...
public interface InscriptionFormationRepository extends JpaRepository<InscriptionFormation, Long> {
    ...
    long countByFormationIdAndStatutNot(Long formationId, StatutInscription statut);

    boolean existsByUser_IdAndFormation_Id(Long userId, Long formationId);

    boolean existsByUser_IdAndFormation_IdAndStatutNot(Long userId, Long formationId, StatutInscription statut);
